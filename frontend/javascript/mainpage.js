const apiKey = process.env.NEWSDATAKEY; // Replace with your NewsData.io API key
...
async function fetchArticles(category) {
  const url = `https://newsdata.io/api/1/latest?apikey=${apiKey}&q=${encodeURIComponent(category)}&language=en`;
...
function createNewsCard(article) {
  ...
  const image_url = article.image_url && (article.image_url !== null || article.image_url !== '[Article Image]' ? article.image_url : 'imgs/test.png');
  ...
  const source_icon = article.source_icon ? article.source_icon : 'imgs/test.png'; // Fallback to a placeholder icon
  ...
  const finalImageUrl = image_url ? (image_url.includes('600px') ? image_url : resizeImage(image_url, 600)) : 'imgs/test.png';
